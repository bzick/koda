...
class Error {
	...
	public static function filteringFailed(ArgumentInfo $arg, $filter, \Exception $error = null) {
		if($error) {
			$ex = new InvalidArgumentException("Error occurred while filtering of the argument $arg: {$error->getMessage()}", 0, $error);
		} else {
			$ex = new InvalidArgumentException("Argument $arg has invalid value. Require " . $arg->filters[$filter]['original'], 0, $error);
		}
		$ex->argument = $arg;
		$ex->filter   = $filter;
		return $ex;
	...
	public static function injectionFailed(ArgumentInfo $arg, \Exception $error) {
		$ex = new InvalidArgumentException("Injection of object {$arg->inject} failed into $arg: {$error->getMessage()}", 0, $error);
		$ex->argument = $arg;
	...
	public static function factoryFailed(ArgumentInfo $arg, \Exception $error) {
		$ex = new InvalidArgumentException("Object creation of {$arg->class} failed for $arg: {$error->getMessage()}", 0, $error);
		$ex->argument = $arg;
		$ex->filter   = self::FILTER_INJECT;
		return $ex;
	}
